...
def _is_label_only_value(value):
    ...
    if re.search(r"[\u4e00-\u9fff]", v) and not re.search(r"[A-Za-z0-9]", v):
        return True
    n = re.sub(r"\s+", " ", v).strip().lower().strip(" :|-.()")
    return n in {
        "principal or seller", "party", "party/intermediate consignee",
        "notify party", "notify party/intermediate consignee", "notify",
        "pol", "pod", "shipper", "consignee", "port of loading",
        "port of discharge", "load port", "discharge port",
        "no. of containers", "no of containers", "total containers",
        "container count", "total container",
    }
...
def extract_gross_weight(text):
    ...
    # OCR fallback:
    # e.g. "TOTAL Gross WeightII(KGS): 359,415 KG"
    # where junk characters appear between "Weight" and the unit.
    for line in lines:
        normalized = re.sub(r"\s+", " ", line).strip()
        if re.search(r"gross\s*(?:weight|wt)", normalized, flags=re.IGNORECASE):
            numbers = re.findall(
                r"\d+(?:,\d{3})*(?:\.\d+)?",
                normalized
            )
