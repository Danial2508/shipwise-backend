...
# =========================================================
# BILINGUAL / PLACEHOLDER VALUE HELPERS
# =========================================================

def is_translation_or_placeholder_line(value):
    """Identify bilingual labels/placeholders that are not real field values."""
    if value is None:
        return False
    ...
    # Pure Chinese/bilingual label fragments.
...
def clean_extracted_candidate(value):
    """
    Keep the original clean_value() untouched.
    Only fix bilingual/placeholder candidates before passing them to it.
    """
    if value is None:
        return None
    ...
    if "|" in raw:
        ...
        if parts and is_translation_or_placeholder_line(parts[0]):
            for part in parts[1:]:
                if part and not is_translation_or_placeholder_line(part):
                    return clean_value(part)
        # Preserve baseline pipe behavior for normal values.
        return clean_value(raw)

    if is_translation_or_placeholder_line(raw):
        return None
...
def find_inline_value(text, labels):
    ...
    for line in lines:
        ...
        for label in sorted_labels:
            ...
            if match:
                value = clean_extracted_candidate(match.group(1))
    ...
    for index, line in enumerate(lines):
        ...
        if label_matches(line, labels):
            ...
            for next_line in lines[index + 1:]:
                ...
                # Stop if another real field label is encountered.
                if is_any_known_label(next_line):
                    break

                # Skip bilingual/placeholder lines and continue to the
                # actual value, e.g.:
                # Shipper
                # (发货人)
                # APRIL FINE PAPER...
                if is_translation_or_placeholder_line(next_line):
                    continue

                value = clean_extracted_candidate(next_line)
    ...
    for line in lines:
        ...
        if "|" in line:
            ...
            if label_matches(label, NOTIFY_LABELS):
                return clean_extracted_candidate(value)
